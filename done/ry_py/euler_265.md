# Solution to Python problem 265
...
def S(n):
    ...
    for bn in binary_strings:  # for each binary number
        ...
        if bzero != bn: bin_d[bn].add(bzero)  # add to bin_d if distinct from bn
        bone = (bn[1:]+'1')  # remove first char, add 1
        if bone != bn: bin_d[bn].add(bone)  # add to bin_d if disctinc from bn
