...
If B contains more elements than C then S(B) > S(C). For example,
...
def is_special(a):
    for first in range(1, len(a)):
        for i in itertools.combinations(a, first):
            sb = set(i)
            sc = a - set(sb)
            for sec in range(1, len(sc) + 1):
                for j in itertools.combinations(sc, sec):
                    scccc = set(j)
                    if (
                        sum(i) == sum(scccc)
                        or (first > sec and sum(i) < sum(j))
                        or (first < sec and sum(i) > sum(j))
                    ):
                        return False
    return True
...
not_special = {81, 88, 75, 42, 87, 84, 86, 65}
assert not is_special(not_special)
special = {157, 150, 164, 119, 79, 159, 161, 139, 158}
