...
def create_markdown(problem_number: int):
    """
    Creates a markdown file where imported functions are expanded to show
    their source code if they are imported from pupy.* and are explicit
    imports (no {@code import *}).
...
def create_md_file(problem_number: int, solution_code: str, dependencies: list):
    ...
    :param dependencies: Functions we imported from our own pupy
